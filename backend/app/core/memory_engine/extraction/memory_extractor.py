...
from app.core.memory_engine.schemas.memory_item import MemoryItem, MemoryType
...
OPTIONAL: If you can infer the user's emotional stats from the messages, include (otherwise omit):
"stats": {{
  "anxiety": 0,
  "paralysis": 0,
  "optimism": 0,
...
(Each stat should be a number from 0-100, default to 0 if uncertain)
"""
    return prompt.strip()
...
def extract_memory(
    ...
    try:
        ...
        logger.warning("LLM structured_generate failed: %s - returning empty result", str(e))
        # Return empty result on LLM failure
        return MemoryOutput(
            preferences=[],
            patterns=[],
            facts=[],
    ...
    raw_stats = raw_json.get("stats", {})
    
    # Safely parse stats with error handling
    try:
        stats = UserStats(
            anxiety=float(raw_stats.get("anxiety", 0)),
            paralysis=float(raw_stats.get("paralysis", 0)),
            optimism=float(raw_stats.get("optimism", 0)),
            stress=float(raw_stats.get("stress", 0)),
        )
    except (ValueError, TypeError) as e:
        # If stats parsing fails, use default values
        stats = UserStats()
